...
import style from "./AntMenu.less";
...
const SideMenu = props => {
  ...
  const MenuDetail = props.menuData.map(menuItem => {
    if (!menuItem.subMenu) {
      return (
    ...
    return (
      ...
      >
        {menuItem.subMenu.map(subMenu => (
          <Menu.Item key={subMenu.id}>{subMenu.text}</Menu.Item>
        ))}
      </SubMenu>
    );
  });
  ...
  return (
    <Menu
      theme="dark"
      defaultSelectedKeys={[String(props.menuData[0].id)]}
      mode="inline"
    >
      {MenuDetail}
    </Menu>
...
class SiderDemo extends Component {
  ...
  render() {
    return (
      <Layout className={style.sidebar}>
        <Sider
          breakpoint="lg"
        ...
        >
          <div
            className={
              this.state.collapsed ? style["logo-collapse"] : style.logo
            }
          >
            <LogoImg />
            {!this.state.collapsed && <span>{this.state.systemName}</span>}
          </div>
          <SideMenu menuData={this.state.menuData} />
        </Sider>
        <Layout>
          <Header className={style.header}>
            {this.state.collapsed && <LogoImg />}
            <Icon
          ...
          <Content className={style.content}>
            ...
            <div className={style.container}>Bill is a cat.</div>
          </Content>
          <Footer className={style["text-center"]}>
